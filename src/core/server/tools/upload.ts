...
const uploadInputSchema = z.object({
  ...
  publishToFilecoin: z.boolean().optional().describe('Whether to publish the file to the Filecoin Network. When true, the file will be published to the Filecoin network, making it publicly accessible. When false (default), the file will only be available within the Storacha network.'),
});

export const uploadTool = (storageConfig: StorageConfig) => ({
  name: 'upload',
  description: 'Upload a file to the Storacha Network. The file must be provided as a base64 encoded string. The file name should include the extension (e.g., "document.pdf") to enable automatic MIME type detection.',
